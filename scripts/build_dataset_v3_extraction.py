...
# ///
"""
Build Deforum Prompt Dataset v3 - EXTRACTION VERSION (No LLM synthesis)

Cleans v2 dataset, extracts cinematic paragraphs from Creative Writing and 
Gutenberg Sci-Fi sources. Fast pipeline (~5 minutes vs hours).

Usage:
    uv run scripts/build_dataset_v3_extraction.py --dry-run --seed 42
    uv run scripts/build_dataset_v3_extraction.py --target Limbicnation/deforum-prompt-lora-dataset-v3 --seed 42
"""
...
# Cinematic vocabulary for augmentation
CINEMATIC_VOCAB = {
    "openings": [
        "A slow push-in reveals", "Through the haze, we see", "The camera drifts across",
        "In the dim light", "Shadows part to show", "A silent movement reveals",
        "Through mist and grain", "The frame holds on"
    ],
    "cameras": [
        "slow tracking shot", "static wide", "handheld drift", "low angle push-in",
        "overhead descent", "orbital movement", "gentle zoom", "whip pan"
    ],
    "lighting": [
        "rim light carving edges", "neon glow cutting through haze",
        "volumetric beams", "practical sources casting pools",
        "backlit silhouette", "dramatic chiaroscuro",
        "atmospheric haze diffusing highlights"
    ],
    "closures": [
        "Film grain texture throughout.", "Atmospheric haze permeates.",
...
def diversify_chiaroscuro(text: str, rng: random.Random, keep_ratio: float = 0.15) -> str:
...
def extract_cinematic_paragraph(text: str, rng: random.Random, max_words: int = 80, min_words: int = 40) -> str:
    """
    Extract and enhance the most cinematic paragraph from source text.
    Fast extraction - no LLM calls.
    """
    # Clean the text
    text = re.sub(r'\s+', ' ', text).strip()
    ...
    # Score each sentence for cinematic density
    scored = []
    for sent in sentences:
        score = score_text_for_visual_density(sent)
        scored.append((sent, score, count_words(sent)))
    
    # Sort by score descending
    scored.sort(key=lambda x: x[1], reverse=True)
    
    # Build paragraph from top sentences
    selected = []
    total_words = 0
    
    for sent, score, word_count in scored:
        if word_count < 5:
            continue
    ...
    if not selected:
        # Fallback: take middle chunk of text
        words = text.split()
        mid = len(words) // 2
        chunk = words[max(0, mid-40):min(len(words), mid+40)]
        return ' '.join(chunk)
    
    # Reassemble in original order for coherence
    result = ' '.join(selected)
    
    # Enhance with cinematic vocabulary if short
    if total_words < 50:
        opening = rng.choice(CINEMATIC_VOCAB["openings"])
        result = f"{opening} {result[0].lower()}{result[1:]}"
    
    # Add atmospheric closure if missing
    if not any(word in result.lower() for word in ['film', 'grain', 'atmospheric', 'haze']):
        closure = rng.choice(CINEMATIC_VOCAB["closures"])
...
def batch_extract_cinematic(
    texts: List[str],
    instruction_hints: List[str],
    rng: random.Random
) -> List[Tuple[str, str]]:
    """Extract cinematic paragraphs - fast, no LLM calls."""
    results = []
    
    for text, hint in tqdm(zip(texts, instruction_hints), total=len(texts), desc="Extracting cinematic text"):
        extracted = extract_cinematic_paragraph(text, rng)
        results.append((hint, extracted))
...
def stage1_clean_v2(rng: random.Random, dry_run: bool = False) -> pd.DataFrame:
    ...
    print(f"  Sarah: {original_sarah_count} → {new_sarah_count} ({100*new_sarah_count/len(result_df) if len(result_df) > 0 else 0:.1f}%)")
    print(f"  Chiaroscuro: {original_chiaroscuro_count} → {new_chiaroscuro_count} ({100*new_chiaroscuro_count/len(result_df) if len(result_df) > 0 else 0:.1f}%)")
    if len(result_df) > 0:
...
def stage2_creative_writing(rng: random.Random, dry_run: bool = False) -> pd.DataFrame:
    """Process Creative Writing ShareGPT dataset with extraction."""
    print("\n" + "="*60)
    ...
    print(f"Selected top {len(top_items)} by visual density")
    
    # Extract cinematic text
    texts = [item['gpt'] for item in top_items]
    hints = [f"Generate a cinematic video prompt for: {item['human'][:80]}" for item in top_items]
    
    extracted = batch_extract_cinematic(texts, hints, rng)
    
    rows = []
    empty_count = 0
    for item, (instruction, response) in zip(top_items, extracted):
        if not response or count_words(response) < 10:
            empty_count += 1
            continue
    ...
    result_df = pd.DataFrame(rows)
    print(f"Stage 2 Results: {len(result_df)} rows ({empty_count} empty/short filtered)")
    if len(result_df) > 0:
        print(f"  Tier distribution: {result_df['tier'].value_counts().to_dict()}")
...
def stage3_gutenberg(rng: random.Random, dry_run: bool = False) -> pd.DataFrame:
    """Process Gutenberg Sci-Fi dataset with extraction."""
    print("\n" + "="*60)
    ...
    try:
        df = conn.execute("SELECT * FROM read_parquet(?) USING SAMPLE 500", [path]).fetchdf()
    except Exception as e:
    ...
    for chunk in top_chunks:
        ...
        hints.append(f"Generate a cinematic video prompt inspired by: {hint[:120]}")
    
    extracted = batch_extract_cinematic(texts, hints, rng)
    
    rows = []
    empty_count = 0
    for chunk, (instruction, response) in zip(top_chunks, extracted):
        if not response or count_words(response) < 10:
            empty_count += 1
            continue
    ...
    result_df = pd.DataFrame(rows)
    print(f"Stage 3 Results: {len(result_df)} rows ({empty_count} empty/short filtered)")
    if len(result_df) > 0:
        print(f"  Tier distribution: {result_df['tier'].value_counts().to_dict()}")
...
def main():
    parser = argparse.ArgumentParser(description="Build Deforum Prompt Dataset v3 (Extraction)")
    ...
    print("="*60)
    print("BUILD DATASET v3 (EXTRACTION - NO LLM)")
    print("="*60)
